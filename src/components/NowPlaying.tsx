...
export default function NowPlaying() {
  ...
  return (
    <div className="flex w-full items-center gap-1 sm:gap-2">
      ...
      {data?.isPlaying && data?.songUrl && <Equalizer />}
      <div className="inline-flex flex-row sm:flex-row w-full max-w-full truncate">
        {data?.songUrl ? (
          <a
            className="text-gray-800 dark:text-gray-200 text-sm max-w-max truncate"
            href={data.songUrl}
            target="_blank"
          ...
          <p className="text-gray-800 dark:text-gray-200 text-sm">Not Playing</p>
        )}
